...
void main() {
  group("NotificationBloc", () {
    ...
    setUp(() {
      ...
      when(
        apiWithUserWithReminder.userStream,
      ).thenAnswer((_) => Stream.fromIterable([userWithReminder]));
      when(
        apiWithUserWithReminder.setHomeworkReminderTime(null),
    ...
    test(
      ...
      () {
        ...
        expect(
          blocWithUserWithReminder.notificationsForHomeworks,
          neverEmits(false),
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        verify(
          api.setHomeworkReminderTime(NotificationsBloc.seedValue),
        ).called(1);
    ...
    test('should call the api when the reminderTime is changed', () async {
      ...
      verify(
        apiWithUserWithReminder.setHomeworkReminderTime(
          const TimeOfDay(hour: 13, minute: 30),
        ),
      );
    });
    ...
    group(".convertReminderTimeToTimeOfDay", () {
      test("convertes correctly", () {
        ...
        bloc = NotificationsBloc(api);
        expect(
          bloc.convertReminderTimeToTimeOfDay("12:30"),
