...
void main() {
  ...
  group('GIVEN a Student with Homework WHEN he opens the homework page', () {
    ...
    Future addToRepository(
      List<StudentHomeworkReadModel> homeworks, [
      InMemoryHomeworkRepository? repo,
    ]) async {
    ...
    test(
      ...
      () async {
        ...
        await addToRepository([
          createHomework(
            id: 'hw',
            todoDate: const Date(year: 2023, month: 11, day: 1),
          ),
        ]);
    ...
    test(
      ...
      () async {
        ...
        await homeworkSortingCache.setLastSorting(
          HomeworkSort.subjectSmallestDateAndTitleSort,
        );
    ...
    test('THEN the current sorting should be cached', () async {
      ...
      expect(
        await homeworkSortingCache.getLastSorting(),
        HomeworkSort.subjectSmallestDateAndTitleSort,
      );
    });
    ...
    test(
      ...
      () async {
        ...
        final Future<StudentHomeworkPageState> findState = bloc.stream
            .firstWhere((state) {
              if (state is Success) {
    ...
    test(
      ...
      () async {
        ...
        final Future<StudentHomeworkPageState> findMatchingState = bloc.stream
            .firstWhere((state) {
              if (state is Success) {
                final sortedOpenHWs = state.open;
                if (sortedOpenHWs.numberOfHomeworks == unsorted.length) {
                  final orderedIds =
                      sortedOpenHWs.orderedHomeworks
                          .map((hw) => hw.id)
                          .toList();
    ...
    test(
      ...
      () async {
        ...
        final homeworks = List.generate(
          3,
          (_) => createHomework(done: false, todoDate: yesterday),
        );
    ...
    test(
      ...
      () async {
        ...
        final Stream<StudentHomeworkPageState> invalidStates = bloc.stream
            .where(
              (state) =>
                  state is Success &&
    ...
    group('CompletedHomeworks lazy loading ', () {
      ...
      test(
        ...
        () async {
          ...
          final Future<StudentHomeworkPageState>
          findExpectedState = bloc.stream.firstWhere(
            (state) =>
                state is Success && state.completed.numberOfHomeworks == 20,
          ); // Skips loading and first Successful state, where the first 10 homeworks were loaded, to get the second successful state.
      ...
      test(
        ...
        () async {
          ...
          final expectedOrderedTitles = List.generate(
            20,
...
class VerboseBlocObserver extends BlocObserver {
  ...
  void onError(BlocBase<dynamic> bloc, Object error, StackTrace stacktrace) {
    log(
      '${bloc.runtimeType} error: $error, stacktrace: $stacktrace',
      error: error,
...
class InMemoryStudentHomeworkPageApi extends StudentHomeworkPageApi {
  ...
  getLazyLoadingCompletedHomeworksController(int nrOfInitialHomeworkToLoad) {
    return repo.getLazyLoadingCompletedHomeworksController(
