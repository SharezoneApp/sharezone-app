...
class MockNextSchooldayCalculator implements NextSchooldayCalculator {
  ...
  Holiday? holiday = Holiday.fromJson(
    jsonEncode({
...
CloudFile randomAttachmentCloudFileWith({
  ...
  return CloudFile.create(
    id: id ?? 'random_id_${randomAlphaNumeric(10)}',
    creatorName: 'Random Assignment Creator Name ${randomAlphaNumeric(5)}',
    courseID: courseId,
    creatorID: 'random_file_creator_id_${randomAlphaNumeric(5)}',
    isPrivate: randomBool(),
    path: FolderPath.fromPathString('/$courseId/${FolderPath.attachments}'),
  ...
  group('HomeworkDialog', () {
    ...
    Future<void> pumpAndSettleHomeworkDialog(
      ...
      if (homework != null) {
        ...
        when(
          homeworkGateway.singleHomework(any, source: Source.cache),
    ...
    testWidgets('edits homework correctly', (tester) async {
      ...
      await tester.enterText(
        find.byKey(HwDialogKeys.titleTextField),
        'New title text',
      );
      ...
      // await tester.tap(find.text('OK'));
      await tester.enterText(
        find.byKey(HwDialogKeys.descriptionField),
    ...
    testWidgets('wants to create the correct homework', (tester) async {
      ...
      await tester.ensureVisible(
        find.byKey(HwDialogKeys.notifyCourseMembersTile, skipOffstage: false),
      );
    ...
    testWidgets('homework lesson chips are not visible in edit mode', (
      ...
      addCourse(courseWith(id: 'foo_course', name: 'Foo course'));
    ...
    testWidgets(
      'when no course is selected then the lesson chips are disabled',
      (tester) async {
        final controller = createController(tester);
        await pumpAndSettleHomeworkDialog(
          tester,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.ensureVisible(
          find.byKey(HwDialogKeys.lessonChipDeleteIcon),
        );
...
class _TestController {
  ...
  List<LessonChip> _getChips() {
    final chips = tester.widgetList<InputChip>(
      find.byType(InputChip, skipOffstage: false),
    );
    return chips
        .map(
          (e) => (
  ...
  Future<void> selectDateManually(Date date) async {
    ...
    tester.widget<DatePicker>(find.byType(DatePicker)).selectDate!(
      date.toDateTime,
    );
    await tester.pumpAndSettle();
  }
