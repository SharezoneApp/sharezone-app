...
void main() {
  group('ListWithBottomThreshold', () {
    ...
    group('scrolling tests:', () {
      ...
      final container = Container(height: childHeight, color: Colors.red);
      final children = List.filled(nrOfChildren, container);
      ...
      Future<void> scrollList(
        WidgetTester tester,
        double distanceFromViewToThreshold, [
        Widget? list,
      ]) async {
        await tester.drag(
          find.byWidget(list ?? infiniteScrollingList!),
          Offset(0, -distanceFromViewToThreshold),
        );
        await tester.pumpAndSettle();
      }

      Future<void> scrollListPastThreshold(
        WidgetTester tester, [
        Widget? list,
      ]) async {
        await scrollList(
          tester,
          getDistanceFromViewToThreshold(tester) + 1000,
          list,
        );
      }

      void testAndPumpList(
        String description,
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          final list = Directionality(
            textDirection: TextDirection.ltr,
            child: ListWithBottomThreshold(
              loadingIndicator: const SizedBox(height: 50),
              thresholdHeight: listThreshold,
              onThresholdExceeded: () => fired = true,
      ...
      testAndPumpList("should fire only once", (tester) async {
        ...
        final distanceFromViewToThreshold = getDistanceFromViewToThreshold(
          tester,
        );
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          final list = Directionality(
            textDirection: TextDirection.ltr,
            child: ListWithBottomThreshold(
              loadingIndicator: const SizedBox(height: 5000),
              thresholdHeight: listThreshold,
              onThresholdExceeded: () => fired = true,
