...
void main() {
  group('grade types', () {
            ...
            subjects: [
              subjectWith(
                ...
                grades: [
                  ...
                  gradeWith(id: const GradeId('bar'), value: 3),
                ],
              ),
            ],
          ),
      ...
      () {
        ...
        for (final gradeType in controller.getPossibleGradeTypes()) {
          if (gradeType.predefinedType != null) {
            expect(
              () => controller.deleteCustomGradeType(gradeType.id),
      ...
      () {
        ...
        controller.createTerm(
          termWith(
            ...
            subjects: [
              subjectWith(
                grades: [gradeWith(type: const GradeTypeId('foo'), value: 3)],
              ),
            ],
          ),
        );
    ...
    test(
      'A custom grade type can be deleted if it is not assigned to anything',
      () {
        final controller = GradesTestController();
    ...
    test(
      ...
      () {
        ...
        final gradeTypeIds = controller.getPossibleGradeTypes().map(
    ...
    test(
      'Trying to edit a predefined grade type will throw an ArgumentError',
      () {
        final controller = GradesTestController();

        for (final gradeType in controller.getPossibleGradeTypes()) {
          expect(
            () => controller.editCustomGradeType(
              gradeType.id,
              displayName: 'foo',
            ),
            throwsArgumentError,
    ...
    test('One can change the name of a custom grade type', () {
      ...
      expect(
        controller.term(const TermId('foo')).finalGradeType.displayName,
        'bar',
      );
    });
    test(
